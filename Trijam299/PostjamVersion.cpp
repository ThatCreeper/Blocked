...
static Texture2D GetMapTexture() {
	switch (s.map) {
	case 0:
		return s.t.map1;
	case 1:
...
static Texture2D *GetTexture(int level) {
	switch (level) {
	case 1:
		return &s.t.n1;
	case 2:
...
static void OpenTileCorner(int x, int y) {
	if (x < 0 || y < 0)
		return;
...
static void OpenTile(int x, int y) {
	if (x < 0 || y < 0)
		return;
	if (x >= MAP_WIDTH || y >= MAP_HEIGHT)
		return;
